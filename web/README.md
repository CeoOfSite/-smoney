...
**Redis (optional, recommended on Render with multiple instances or after restarts):** set **`REDIS_URL`** to your [Render Key Value](https://docs.render.com/deploy-redis) URL. **Internal** URL (`redis://red-…:6379`) works only **on Render** and only if the Key Value and **web service are in the same region**; it will **not resolve on your laptop** (`getaddrinfo ENOTFOUND`). For local dev, leave `REDIS_URL` unset or use the **external** URL (`rediss://…`) after enabling external access and allowlisting your IP. In the Dashboard: **Web Service** → **Environment** → **`REDIS_URL`** from the Key Value **Connect** tab. In a [Blueprint](https://docs.render.com/docs/blueprint-spec), use `fromService`: `type: keyvalue`, `name: <your-kv-name>`, `property: connectionString`.

**Inventory cache tracing:** set **`INVENTORY_CACHE_LOG=1`** on the web service to log Redis/memory **GET HIT/MISS**, **SET**, guest TTL expiry, SWR stale decisions, and Steam foreground/background fetches (prefix `[inv-cache]`). In development, these logs are on by default unless `INVENTORY_CACHE_LOG=0`.
