...
**Redis (optional, recommended on Render with multiple instances or after restarts):** set **`REDIS_URL`** to your [Render Key Value](https://docs.render.com/deploy-redis) internal URL (`redis://` or `rediss://`). In the Render Dashboard: open your **Web Service** → **Environment** → add **`REDIS_URL`** (paste from the Key Value instance’s **Internal Redis URL**, or use **Add from** → your Key Value). Inventory snapshots and refresh cooldowns then use Redis; if `REDIS_URL` is unset, the app keeps the previous in-memory cache behavior. In a [Blueprint](https://docs.render.com/docs/blueprint-spec), you can wire it with `fromService`: `type: keyvalue`, `name: <your-kv-name>`, `property: connectionString`.
