...
- **Режим Instant Sell** (отдельная **Фаза 9**): отдельный сценарий «продать скины за деньги» по UX, близкому к [swap.gg/sell](https://swap.gg/sell): слева — выбранные предметы и сетка инвентаря, справа — итог **только в USD ($)** и выбор способа выплаты из фиксированного списка: **BTC, ETH, PayPal, Wise, USDT, USDC** (без отображения сумм в крипте на витрине, если не оговорено иное — ориентир **все суммы и цены в долларах**). Реализация выплат и комплаенс — по подфазам внутри Фазы 9.

Вне MVP (явно отложить): боты, **полностью автоматический** обмен в Steam, вебсокеты там, где достаточно **периодического опроса**, мультивалютная аналитика, сложные роли. **Instant Sell** — не «вне MVP», а **отдельная фаза** после базового обменника (можно планировать сразу после стабильного инвентаря и цен).
...
- **Поддержка (связь с пользователями, референс cs.money):**
  - **SupportThread** (или **SupportTicket**) — привязка к **`User`** (`steamId` / `userId`), `createdAt`, `updatedAt`, статус (`open` / `closed` по желанию), флаг **«есть непрочитанное от пользователя»** для админки (или считать по сообщениям).
  - **SupportMessage** — `threadId`, текст, `createdAt`, автор: **`user` | `admin`**, опционально `readByAdminAt` / `readByUserAt` для MVP достаточно метки «прочитано админом» на входящих от пользователя.
  - Один пользователь — **один активный тред** или **несколько тредов** (на усмотрение: проще один открытый диалог на пользователя в MVP). Переписка **только внутри сайта** (без обязательной интеграции почты на первом этапе); уведомления на email/Telegram — **вне MVP** или отдельным этапом.
- **Instant Sell (Фаза 9):**
  - **InstantSellOrder** (или **InstantSellRequest**) — `userId` / `creatorSteamId`, **снимок предметов** (как у TradeItem: `assetId`, `marketHashName`, фаза, wear, float, **цена USD** на момент заявки), **сумма итого USD**, **способ выплаты** (enum: `BTC` | `ETH` | `PAYPAL` | `WISE` | `USDT` | `USDC`), **контактные/платёжные реквизиты** (email для PayPal, реквизиты Wise, адрес кошелька для крипты — хранение и маскирование по политике безопасности), статус (`pending`, `processing`, `paid`, `cancelled`, `rejected` — уточнить список), `createdAt`, `updatedAt`, опционально `adminNotes`.
  - Отдельно от **Trade**: это не swap предметов, а **заявка на выкуп за USD** с выбранным каналом выплаты.
...
- **Кнопка поддержки (как на cs.money):** плавающая кнопка **справа внизу** на основных страницах (глобальный layout). По нажатию — **панель/модалка**: список сообщений в треде пользователя + поле ввода и отправка. **Доступ:** только для **авторизованных** (привязка к `User` / `steamId`); для гостей — текст «Войдите через Steam, чтобы написать в поддержку» или скрыть кнопку (зафиксировать в реализации). API: `GET/POST` сообщений в своём треде, валидация и rate limit на сервере.
...
- Список заявок, фильтр по статусу, карточка заявки, смена статуса, заметки. После появления **Фазы 9** — аналогичный раздел по **Instant Sell**-заявкам (состав, сумма USD, способ выплаты, реквизиты, статус).
...
- **Поддержка / переписка с пользователями (связка с фазой 5):**
  - Отдельный раздел в админке: **список тредов** (по пользователям), видно **кто писал**, время последнего сообщения, **бейдж непрочитанного** (новые сообщения от пользователя с момента последнего просмотра админом).
  - **Экран диалога:** лента сообщений выбранного пользователя + поле **ответа**; каждый пользователь ведётся **отдельной перепиской** (один тред на пользователя в MVP — достаточно).
  - Обновление списка и чата: **периодический опрос** (polling раз в N секунд) или ручное «Обновить»; полноценные **вебсокеты** — вне MVP, если не требуются.
  - Опционально позже: закрытие треда, экспорт, уведомление владельцу на email при новом сообщении пользователя.
...
### Фаза 9 — Режим Instant Sell (референс swap.gg/sell)

**Цель:** отдельный поток на сайте — пользователь **продаёт** свои скины за **доллары (USD $)** и выбирает **способ получения выплаты**. UX и компоновка экрана — **как на [swap.gg/sell](https://swap.gg/sell)** (верхняя зона «что продаёте» + сетка инвентаря слева, **правый сайдбар** с итогом и шагом выбора способа оплаты / «Далее»).

**Валюта отображения:** **только USD** для всех цен предметов, промежуточных сумм и итога на экране (без переключателя валют в этом режиме, если не оговорено отдельно).

**Способы выплаты (фиксированный набор кнопок-плиток, как у референса):**

| Код | Пользовательский ярлык |
|-----|-------------------------|
| BTC | Bitcoin |
| ETH | Ethereum |
| PAYPAL | PayPal |
...
После выбора метода — форма сбора **минимально необходимых реквизитов** (адрес кошелька для BTC/ETH/USDT/USDC; email или идентификатор для PayPal; реквизиты Wise — по согласованному шаблону). Валидация форматов на сервере; **не** хранить лишнее в открытом виде (маскирование в админке, доступ только `isAdmin`).

**Логика цен и отбор предметов:** переиспользовать **те же правила**, что и для обмена: трейдлок, порог минимальной цены, ручные цены владельца по `assetId` — что применимо к **инвентарю пользователя** в этом режиме. Сервер пересчитывает **итог USD**; клиенту не доверять.

**API (черновик):** `POST /api/instant-sell` — создание заявки (авторизация обязательна, trade-ссылка как для инвентаря); `GET /api/instant-sell/me` — список своих заявок. Админка: список всех Instant Sell заявок, смена статуса, просмотр реквизитов, заметки (можно объединить с существующим разделом заявок или вынести отдельным пунктом меню).

**Выплаты (MVP — поэтапно):**

1. **MVP-уровень A:** заявка создаётся в БД, владелец **вручную** проводит выплату вне сайта и переводит статус в админке («оплачено» / «отклонено»). Уведомления пользователю — по почте или через поддержку — **опционально**, вторым шагом.
2. **Уровень B (вне минимального MVP или отдельный подпроект):** интеграции с платёжными/крипто-провайдерами, KYC, автоматические выплаты — только после юридической оценки и выбора провайдеров.

**Риски и комплаенс (заложить в оценку фазы):** мошенничество, чарджбеки (PayPal / Wise), корректность крипто-адресов, **AML/KYC** в зависимости от юрисдикции; лимиты на размер сделки и ручная модерация крупных заявок на первых порах.

**Зависимости:** стабильный **вход Steam**, **инвентарь гостя**, **ценник из БД** (Фазы 2–5). Реализацию можно **планировать сразу после** базовой страницы обмена или **параллельно** админке при отдельной оценке сроков.
...
- **Instant Sell** — ошибочные реквизиты и невосстановимые крипто-переводы: двойное подтверждение адреса в UI, предупреждения в тексте заявки; крупные суммы — ручная проверка до смены статуса.
